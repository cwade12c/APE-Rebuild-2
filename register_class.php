...
function registerStudent($ewuId, $firstName, $lastName, $ewuEmail) {
    if(!empty($firstName) && !empty($lastName) && $ewuId && $ewuEmail) {
        ...
        $sql = executeQuery("INSERT INTO accounts(id, type, f_name, l_name, email) values('$ewuId', '$type', '$firstName', '$lastName', '$ewuEmail');");
    }
    else {
...
function registerStudentsFromCsv() {
    ...
    while ( ($line = fgets($csv)) !== false) {
        ...
        if(count($studentProperties) == 4) {
            registerStudent($studentProperties[0], $studentProperties[1], $studentProperties[2], $studentProperties[3]);
        }
    }
}
